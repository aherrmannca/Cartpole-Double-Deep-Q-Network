...
class DQN():
    ...
    def _create_model(self):
        ...
        # PLACEHOLDERS
        # Input = 4 frames : 160x128x4
        self.X = tf.placeholder(tf.uint8, shape=[None, 160, 128, 4], name='X')
        ...
        # h = 160 / 2*2 and w = 128 / 2*2
        flat = tf.reshape(max_pool_2, [-1, 40 * 32 * 64])
...
class StateProcessor():
    ...
    def __init__(self):
        ...
        with tf.variable_scope("state_processor"):
            ...
            self.output = tf.image.crop_to_bounding_box(self.output, 0, 0, 200, 160)
            self.output = tf.image.resize_images(self.output, [160, 128], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
