...
class DQN():
    ...
    def _create_model(self):
        ...
        # PLACEHOLDERS
        # Input = 4 frames, 800x1200
        self.X = tf.placeholder(tf.uint8, shape=[None, 100, 300, 4], name='X')
        ...
        # h = 100 / 2*2 and w = 300 / 2*2
        flat = tf.reshape(max_pool_2, [-1, 25 * 75 * 64])
    ...
    def update(self, sess, s, a, y):
        ...
        print("y_pred: ", sess.run(self.y_pred, feed_dict={self.X: s, self.phase: False}).shape)
...
class StateProcessor():
    ...
    def __init__(self):
        ...
        with tf.variable_scope("state_processor"):
            ...
            self.output = tf.image.crop_to_bounding_box(self.output, 300, 0, 400, 1200)
            self.output = tf.image.resize_images(self.output, [100, 300], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    def process(self, sess, state):
        ...
        Returns:
            A processed [100, 300] state representing grayscale values.
        """
...
# Possible actions to take. 0 = left, 1 = right
action_list = [0, 1]
